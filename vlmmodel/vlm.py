...
def preprocess_v1(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(rounds):
            ...
            # a=tokenizer_image_token(parts[0], tokenizer, return_tensors='pt')
            # b=tokenizer_image_token(parts[1], tokenizer, return_tensors='pt')
            # c=tokenizer_image_token(rou, tokenizer, return_tensors='pt')
            # d=tokenizer_image_token(sep, tokenizer, return_tensors='pt')
            ...
            # print(tokenizer.decode(791))
            # print(tokenizer.decode(578))
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
                 tokenizer: transformers.PreTrainedTokenizer,
                 data_args: DataArguments,data_len=0):
...
def make_supervised_data_module_local(tokenizer: transformers.PreTrainedTokenizer,
    ...
    training_datasets = LazySupervisedDataset(data_path=data_args.data_path+"llava_train_QCM-A.json",tokenizer=tokenizer,
                                data_args=data_args)
    
    global_test_dataset = LazySupervisedDataset(data_path=data_args.data_path+"llava_test_QCM-A.json",tokenizer=tokenizer,
                                data_args=data_args)
...
def init_vlm_model_test(training_args, model_args,model_path):
    model = PureLlamaForCausalLM.from_pretrained(
        model_path,
        # 其他参数根据需要添加
    )
